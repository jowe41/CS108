...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		for (TPoint point:points) {
			skirt[point.x] = skirt[point.x] < point.y ? skirt[point.x] : point.y;
		}
		next = null;
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] nextPoints = flipDiagonal(body);
		nextPoints = flipVertical(nextPoints, width);
		Piece nextPiece = new Piece(nextPoints);
		return nextPiece; // YOUR CODE HERE
	}

	private TPoint[] flipVertical(TPoint[] nextPoints, int height2) {
		// TODO Auto-generated method stub
		TPoint[] filpedPiece = new TPoint[nextPoints.length];
		for (int i = 0; i < nextPoints.length; i++) {
			filpedPiece[i] = new TPoint(nextPoints[i].x, height2 - 1 - nextPoints[i].y);
		}
		return filpedPiece;
	}


	private TPoint[] flipDiagonal(TPoint[] body2) {
		TPoint[] filpedPiece = new TPoint[body2.length];
		for (int i = 0; i < body2.length; i++) {
			filpedPiece[i] = new TPoint(body2[i].y, body2[i].x);
		}
		return filpedPiece;
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		...
		Piece nextP;
		for (;;) {
			nextP = curr.computeNextRotation();
